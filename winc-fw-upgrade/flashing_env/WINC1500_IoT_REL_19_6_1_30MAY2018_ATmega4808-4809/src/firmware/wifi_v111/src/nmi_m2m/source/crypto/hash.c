...
#ifdef __BUILD_ROM_LIB__
#define MD4_SEC  __attribute__((externally_visible, noinline))
#else
...
{
#ifdef __HW_SHA256_ENGINE__
	if(u8Flags & SHA_FLAGS_INIT)
		Sha256_Init_hw(&pstrSha256Cxt->strSha);
	if(u8Flags & SHA_FLAGS_UPDATE)
		Sha256_Update_hw(&pstrSha256Cxt->strSha, pu8Data, u32DataLength);
	if(u8Flags&SHA_FLAGS_FINISH)
		Sha256_Finish_hw(&pstrSha256Cxt->strSha, pu8Digest);
#else
