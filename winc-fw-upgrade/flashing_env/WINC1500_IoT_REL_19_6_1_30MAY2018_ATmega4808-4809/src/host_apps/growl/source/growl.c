...
#define GROWL_CONNECT_RETRY					2
#define GROWL_DNS_RETRY						2


#define PROWL_DOMAIN_NAME				"api.prowlapp.com"//"espringwolfedemo.servicebus.windows.net"//"www.google.com.eg"//"carol.sni.velox.ch"//
#define PROWL_CLIENT_STRING_ID			"prowl"//"google"//"velox"//
#define PROWL_API_KEY_SIZE				40

#define NMA_DOMAIN_NAME					"www.notifymyandroid.com"//"hb.zcontrolcloud.com"//"apismart.jd.com"
#define NMA_CLIENT_STRING_ID			"notifymyandroid"//"zcontrolcloud"//"apismart"
...
void GROWL_APPSocketEventHandler(SOCKET sock, uint8 u8Msg, void * pvMsg);
/*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*
PRIVATE DATA TYPES
*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*/
 ...
 void GROWL_APPSocketEventHandler(SOCKET sock, uint8 u8Msg, void * pvMsg)
{
	...
	if(u8Msg == SOCKET_MSG_CONNECT)
	{
		...
		if(pstrConnect->s8Error == 0)
		{
			...
			M2M_DBG("%s Sending <%u> bytes\n",(u8ClientID == NMA_CLIENT) ? "NMA" : "PROWL", u16MsgSize);
	...
	else if(u8Msg == SOCKET_MSG_SEND)
	{
		...
		{
			M2M_DBG("%s SENT <%u> BYTES\n",(u8ClientID == NMA_CLIENT) ? "NMA" : "PROWL", s16Sent);
		}
	}
}
/*********************************************************************
 ...
 void AppServerCb(uint8* pu8HostName, uint32 u32ServerIP)
{
	...
	if(u32ServerIP != 0)
	{
		...
		if(connect(pstrNotification->Socket, (struct sockaddr*)&strAddr, sizeof(struct sockaddr_in)))
		{
			close(pstrNotification->Socket);
			pstrNotification->Socket = 0xFF;
			pstrNotification->u8State = GROWL_STATE_IDLE;
		...
		if(pstrNotification->u8State == GROWL_STATE_IDLE)
		{
				...
				(m2m_strlen(pu8Event) < GROWL_EVENT_MAX_LENGTH))
			{
				...
				if(pstrNotification->Socket >= 0)
				{
					...
					if(u8ClientName == NMA_CLIENT)
					{
						s8DnsRet = gethostbyname((uint8*)NMA_DOMAIN_NAME);
					}
					else if(u8ClientName == PROWL_CLIENT)
					{
						s8DnsRet = gethostbyname((uint8*)PROWL_DOMAIN_NAME);
					}

					if(s8DnsRet == M2M_SUCCESS)
					{
						pstrNotification->u8State = GROWL_STATE_REQ_PENDING;
					}
					else
					{
						M2M_ERR("gethostbyname err = %d\n",s8DnsRet);
						retVal = -1;
					}
				}
				else
