...
#define MD5_INTERMEDIATE(A,B,C,D,W,K,S)	\
{	\
	W 	+= A + K;				\
	W 	= B + SHA_ROTL(W, S);	\
	A 	= D;					\
...
#define SHA1_INTERMEDIATE(A,B,C,D,E,W,K)	\
{	\
	W += SHA_ROTL(A,5) + E  + K;	\
	E = D;							\
	D = C;							\
...
static void MD5ProcessBlock(uint32 *pu32Md5State,const uint8* pu8MessageBlock)
{
	...
	{
		W = F(B,C,D) + X[u8Idx];
		MD5_INTERMEDIATE(A, B, C, D, W, T[u8Idx], S[u8Idx]);
	}
...
static void SHA1ProcessBlock(uint32* pu32Sha1State, const uint8* pu8MessageBlock)
{
	...
	*/
	...
	{
		W[t] += SHA1_CH(B, C, D);
		SHA1_INTERMEDIATE(A, B, C, D, E, W[t], K[0])
	}

	for (t = 20; t < 40; t++) 
	{
		W[t] += SHA_PARITY(B, C, D);
		SHA1_INTERMEDIATE(A, B, C, D, E, W[t], K[1])
	}

	for (t = 40; t < 60; t++) 
	{
		W[t] += SHA1_MAJ(B, C, D);
		SHA1_INTERMEDIATE(A, B, C, D, E, W[t], K[2])
	}

	for (t = 60; t < 80; t++) 
	{
		W[t] += SHA_PARITY(B, C, D);
		SHA1_INTERMEDIATE(A, B, C, D, E, W[t], K[3])
	}
 ...
 )
{
	if(pstrMD5Context != NULL)
	{
		...
		if(u8Flags & 1)
		{
			...
			/* Define MD5 specific parameters. */
			pstrMD5Context->fpHash       = MD5ProcessBlock;
			pstrMD5Context->u8BlockSize  = SHA_BLOCK_SIZE;
			pstrMD5Context->u8DigestSize = MD5_DIGEST_SIZE;
