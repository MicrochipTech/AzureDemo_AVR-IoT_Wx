...
#define SUBPART_BOUNDARY "----WebKitFormBoundary06f6g54NVbSieT6y"
#define HTTP_POST_REQ_PART1 \
	"POST / HTTP/1.1\r\n"\
	"Host: %s\r\n"\
	"Content-Type: multipart/form-data; boundary=" SUBPART_BOUNDARY "\r\n"\
	"Content-Length: %d\r\n"

#define HTTP_POST_REQ_PART2 \
	"\r\n"\
	"--" SUBPART_BOUNDARY "\r\n"\
	"Content-Disposition: *name=\"file\"; filename=\"%s\"\r\n"\
	"Content-Type: application/octet-stream\r\n"\
	"\r\n"

#define END_SUBPART_BOUNDARY \
	"\r\n"\
	"--" SUBPART_BOUNDARY "--\r\n"
...
static void httpClientRecvCallbackDl
	(
	tstrHTTPClient	*pstrHTTPClient,
	uint8 			*pu8RxBuffer,
	sint16 			s16DataSize
	);
static void httpClientRecvCallbackUl
	(
	tstrHTTPClient	*pstrHTTPClient,
	uint8 			*pu8RxBuffer,
	sint16 			s16DataSize
	...
	)
{
	if(pstrHTTPClient->enuAction == HTTP_CLIENT_ACTION_DOWNLOAD) {
		httpClientRecvCallbackDl(pstrHTTPClient, pu8RxBuffer, s16DataSize);
	} else {
		httpClientRecvCallbackUl(pstrHTTPClient, pu8RxBuffer, s16DataSize);
	}
}
	...
	)
{
	if((pstrHTTPClient != NULL) && (pu8RxBuffer != NULL) && (s16DataSize > 0))
	{
		...
		*/
		...
		if((pu8HttpResp != NULL) && (u32DataSize != 0))
		{
			uint32	u32Rem;
	...
	)
{
			...
			if(pstrHttpHdr->bIsComplete)
			{
				...
				*/
				...
				if(u32HttpRspStatus == 200) {
					...
					pstrHTTPClient->fpCb(NULL, (sint16)HTTP_CLIENT_FINISHED, pstrHTTPClient->u32CbArg);					
				}
				else
				{
					M2M_INFO("HTTP Download Error %lu\n", u32HttpRspStatus);
	...
	else if(s16DataSize <= 0) 
	{
		M2M_INFO("Socket Error %ld\n", s16DataSize);
		if(pstrHTTPClient->fpCb != NULL)
		{
			pstrHTTPClient->fpCb(NULL, HTTP_CLIENT_CONNECTION_ERROR, pstrHTTPClient->u32CbArg);
		}					
...
)
{
	if(err == SOCK_ERR_NO_ERROR)
	{
		...
		else
		{
			...
			u32ContentLength = sprintf(acHttpREQ, HTTP_POST_REQ_PART2 END_SUBPART_BOUNDARY, pstrHTTPClient->acFileName);
			//printf(acHttpREQ);
			u32ContentLength += pstrHTTPClient->u32UploadSize;			
			//u32ContentLength += 10000;
			u32HttpReqSize = sprintf(acHttpREQ, HTTP_POST_REQ,
				pstrHTTPClient->acHostName, u32ContentLength, pstrHTTPClient->acFileName);
			//printf(acHttpREQ);
			//printf("%d\n", u32ContentLength);
...
)
{
	...
	if(pstrHTTPClient->fpCb != NULL)
	{
		...
		do 
		{
			...
			{
				u16TxSize = sizeof(pstrHTTPClient->au8TxBuffer);
				if(u16TxSize > pstrHTTPClient->u32UploadSize)
					u16TxSize = pstrHTTPClient->u32UploadSize;
					
				pstrHTTPClient->fpCb(pstrHTTPClient->au8TxBuffer, u16TxSize, pstrHTTPClient->u32CbArg);
			}
					
			if((send_ret = send(pstrHTTPClient->sock, pstrHTTPClient->au8TxBuffer, u16TxSize, 0)) != 0)
			{
			...
			{
				pstrHTTPClient->bIsTxPending = 0;
				pstrHTTPClient->u32TxSize = 0;
				if(pstrHTTPClient->u32UploadSize > 0) {
					pstrHTTPClient->u32UploadSize -= u16TxSize;				
					if(pstrHTTPClient->u32UploadSize == 0)
					{
						strcpy(pstrHTTPClient->au8TxBuffer, END_SUBPART_BOUNDARY);
						u16TxSize = sizeof(END_SUBPART_BOUNDARY)-1;
						if((send_ret = send(pstrHTTPClient->sock, pstrHTTPClient->au8TxBuffer, u16TxSize, 0)) != 0)
						{
							pstrHTTPClient->bIsTxPending	= 1;
							pstrHTTPClient->u32TxSize		= u16TxSize;
...
static sint8 HttpClientParseURL(tstrHTTPClient *pstrHTTPClient, char *pcURL)
{
	...
	if(s8Error == 0)
	{
		...
		if(pcTmp < (pcURL + u16URLSize))
		{
			...
			{
				pcTmp = HttpstrTOK(NULL, (char*)")");
				strcpy(&pstrHTTPClient->acFileName[0], pcTmp);
			}
