...
set TGTIN=%5
set PRTIN=%8
set IMAGE_FILE_IOT=Tools\ATmega4808_xplained_pro_IOT\wifi-iot-node-demo-atmega4808.elf
...
echo.
echo Calling...
echo %atprogram% -t %TOOL% %TOOLSN% -i UPDI -d %MCU% chiperase
echo %atprogram% -t %TOOL% %TOOLSN% -i UPDI -d %MCU% program ^^
echo     -f %IMAGE_FILE%
echo %atprogram% -t %TOOL% %TOOLSN% -i UPDI -d %MCU% reboot
...
FOR /L %%H IN (1,1,5) DO (
	...
	echo Erasing host...
	%atprogram% -t %TOOL% %TOOLSN% -i UPDI -d %MCU% chiperase
	echo Programming host...
	%atprogram% -t %TOOL% %TOOLSN% -i UPDI -d %MCU% program -f %IMAGE_FILE%
	if a%MCU%a==aATSAM4SD32Ca (
		echo Setting Boot from Flash...
		%atprogram% -t %TOOL% %TOOLSN% -i UPDI -d %MCU% write -fs -o 0 --values 0x42
	)
	echo Resetting host...
	%atprogram% -t %TOOL% %TOOLSN% -i UPDI -d %MCU% reboot
	PING 127.0.0.1 -n 6 >NUL
	%atprogram% -t %TOOL% %TOOLSN% -i UPDI -d %MCU% reset
