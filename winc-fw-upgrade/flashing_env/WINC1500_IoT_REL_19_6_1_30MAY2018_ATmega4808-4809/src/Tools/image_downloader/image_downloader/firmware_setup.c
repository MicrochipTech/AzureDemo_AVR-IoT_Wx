...
sint8 update_look_up_table(uint8* vflash)
{
	...
	if( (magic[0] != PLL_MAGIC_NUMBER) || (magic[1] != xo_offset_magic)) {
		...
		ret = programmer_erase(M2M_PLL_FLASH_OFFSET, M2M_CONFIG_SECT_TOTAL_SZ, vflash);
		...
		ret = programmer_write((uint8*)&data[0], M2M_PLL_FLASH_OFFSET, M2M_CONFIG_SECT_TOTAL_SZ, vflash);
		if(ret!=M2M_SUCCESS) goto ERR;
