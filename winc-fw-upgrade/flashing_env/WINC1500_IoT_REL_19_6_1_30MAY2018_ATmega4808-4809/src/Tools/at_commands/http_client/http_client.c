...
#define HTTP_CLIENT_CONN_RETRY_MAX						(2)
/*!<
*/

enum enum_httpHdrState {
	HTTP_HDR_STATE_IDLE = 0,
	HTTP_HDR_STATE_LINEFEED
};
...
*/
static uint32 HttpClientParseHeaderField(char* pcHeader, uint32* u32StatusCode, uint32* pu32ContentLength)
{
	if(!HttpstrCmpIgnoreCase(pcHeader, (char*)"HTTP/", (uint32)strlen("HTTP/")))
	{
		while(*pcHeader != ' ' && *pcHeader != '\0')
			pcHeader++;
		*u32StatusCode = Ascii2Dec(pcHeader);
	}
	else if(!HttpstrCmpIgnoreCase(pcHeader, (char*)"Content-Length", (uint32)strlen("Content-Length")))
	{
		pcHeader += strlen("Content-Length");
		*pu32ContentLength = Ascii2Dec(pcHeader);
	}
}
	...
	if((pstrHTTPClient != NULL) && (pu8RxBuffer != NULL) && (s16DataSize > 0))
	{
		...
		*/
		if(!pstrHttpHdr->bIsComplete)
		{
			/* Searching.
			*/
			for(uint32 i=0;i<s16DataSize;i++)
			{
				if(pstrHttpHdr->strHttpHdrField.u8State == HTTP_HDR_STATE_IDLE && pu8HttpResp[i] == '\r')
					pstrHttpHdr->strHttpHdrField.u8State = HTTP_HDR_STATE_LINEFEED;
				else if(pstrHttpHdr->strHttpHdrField.u8State == HTTP_HDR_STATE_LINEFEED && pu8HttpResp[i] == '\n')
				{
					if(pstrHttpHdr->strHttpHdrField.u8FieldSize > 0)
					{
						pstrHttpHdr->strHttpHdrField.pu8Field[pstrHttpHdr->strHttpHdrField.u8FieldSize] = '\0';
						HttpClientParseHeaderField(pstrHttpHdr->strHttpHdrField.pu8Field, &pstrHttpHdr->u32HttpRspStatus, &pstrRxMsg->u32ContentLength);
						pstrHttpHdr->strHttpHdrField.u8State = HTTP_HDR_STATE_IDLE;
						pstrHttpHdr->strHttpHdrField.u8FieldSize = 0;
					}
					else
					{
						pstrHttpHdr->bIsComplete = 1;
						pu8HttpResp = (i < s16DataSize - 1) ? &pu8HttpResp[i + 1] : &pu8HttpResp[i];
						break;
					}
				}
				else
				{
					...
					if(pstrHttpHdr->strHttpHdrField.u8FieldSize < HTTP_CLIENT_HEADER_MAX - 1)
					{
						pstrHttpHdr->strHttpHdrField.pu8Field[pstrHttpHdr->strHttpHdrField.u8FieldSize] = pu8RxBuffer[i];
						pstrHttpHdr->strHttpHdrField.u8FieldSize++;
					}
				}
			}
			...
			//	printf("\nHTTP RSP\n\n%s", pstrHttpHdr->pu8Hdr);

				pstrRxMsg->u32RxCount		= 0;

				if(pstrHTTPClient->fpCb != NULL)
					pstrHTTPClient->fpCb(NULL, HTTP_CLIENT_DOWNLOADING, pstrHTTPClient->u32CbArg);

				if(pstrHttpHdr->u32HttpRspStatus == 200)
				{
					...
					u32DataSize -= (pu8HttpResp - pu8RxBuffer);				
				}
				else
				{
					u32DataSize = 0;

					M2M_INFO("HTTP Download Error %lu\n", pstrHttpHdr->u32HttpRspStatus);
					if(pstrHTTPClient->fpCb != NULL)
					{
					...
					if(u32Rem == 0)
					{
						...
						if(*pu8HttpResp == '\r')
						{
							*pu8ChunkLen					= 0;
							...
							pu8HttpResp += 2;
							u32DataSize -= 2;
							u32Rem		= pstrRxMsg->u32ContentLength;
						}
						else
						{
