...
static uint8 gbProwlActive = 0;
static uint8 gbNmaActive = 0;
...
static uint8 gu8ActiveBlink = 0;
static uint8 gu8TimActive = 0;
...
static char gacHttpProvDomainName[] = HTTP_PROV_SERVER_DOMAIN_NAME;
...
{
	uint8 val = 0;
	if (gu8ActiveBlink & BITS_t[LED_GRWOL])
	{
		val = nm_gpio_get_out_state(LED_GRWOL);
		val ^= 1;
		nm_gpio_output_ctrl(LED_GRWOL, val);
	}
	if (gu8ActiveBlink & BITS_t[LED_WIFI])
	{
		val = nm_gpio_get_out_state(LED_WIFI);
		val ^= 1;
		nm_gpio_output_ctrl(LED_WIFI, val);
	}
}
static void led_ctl(uint8 led, uint8 state)
{
	switch (state)
	{
	case OFF: /* OFF */
		//P3 |= led;
		nm_gpio_output_ctrl(led, OFF);
		gu8ActiveBlink &= ~BITS_t[led];
		on &= ~BITS_t[led];
		break;
	case ON: /* ON */
		nm_gpio_output_ctrl(led, ON);
		gu8ActiveBlink &= ~BITS_t[led];
		on |= BITS_t[led];
		break;
	case BLINK: /* blink */
	...
	if (gu8ActiveBlink) {
		if (!gu8TimActive) {
			M2M_DBG("BLINK %d\n", gu8ActiveBlink);
			app_os_timer_start(&gstrTimerLed, "App_led", timer_callback,
					APP_TIMER_PERIOD, 1, NULL, 0);
			gu8TimActive = 1;
		}
	} else {
		M2M_DBG("Doze %d\n", gu8ActiveBlink);
		gu8TimActive = 0;
		app_os_timer_stop(&gstrTimerLed);
		m2m_memset((uint8*) &gstrTimerLed, 0, sizeof(gstrTimerLed));
	}
}

static void btn_cb(uint8 u8Btn, uint8 u8Type)
{
	M2M_DBG("btn %d type %d\n",u8Btn,u8Type);
	if (gu8App == APP_WIFI)
	{
		if (u8Type == SHORT_PRESS)
		{
			if (gbWifiConnected == M2M_WIFI_CONNECTED)
			{
#ifdef GROWL
				if ((gbProwlActive == 0) || (gbNmaActive == 0))
				{
					if ((gbProwlActive == 0) && (gbNmaActive == 0))
						...
						genuActReq = (
								(gbProwlActive == 0) ?
										ACT_REQ_START_PROWL_APP :
										ACT_REQ_START_NMA_APP);

					APP_SEM_POST(&gstrAppSem);
				}
#elif defined UDP_TEST
				if(!gbUdpTestActive)
				{
					gbUdpTestActive = 1;
					SocketTest();
...
static void tcp_cb(uint8 cmd)
{
	...
	{
		M2M_INFO("LED 1 ON\n");
		led_ctl(LED_DEMO1,ON);
	}
	if(cmd == 'B')
	{
		M2M_INFO("LED 1 OFF\n");
		led_ctl(LED_DEMO1,OFF);
	}
	if(cmd == 'C')
	{
		M2M_INFO("LED 2 ON\n");
		led_ctl(LED_DEMO2,ON);
	}
	if(cmd == 'D')
	{
		M2M_INFO("LED 2 OFF\n");
		led_ctl(LED_DEMO2,OFF);
	}
	if(cmd == 'E')
	{
		M2M_INFO("LED 3 ON\n");
		led_ctl(LED_DEMO3,ON);

	}
	if(cmd == 'F')
	{
		M2M_INFO("LED 3 OFF\n");
		led_ctl(LED_DEMO3,OFF);
...
static void app_wifi_start_stop_prov(void)
{
	if(gu8App == APP_WIFI) {
...
static void wifi_cb(uint8 u8MsgType, void * pvMsg)
{
	if (u8MsgType == M2M_WIFI_RESP_CON_STATE_CHANGED)
	{
		...
		else if (pstrWifiState->u8CurrState == M2M_WIFI_DISCONNECTED)
		{
			...
			if (gu8App == APP_P2P)
			{
				led_ctl(LED_P2P, BLINK);
			}
			else if (gu8App == APP_AP)
			{
				led_ctl(LED_AP, BLINK);
			}
			else if (gu8App == APP_WIFI)
			{
				led_ctl(LED_WIFI, BLINK);
	...
	else if (u8MsgType == M2M_WIFI_REQ_DHCP_CONF)
	{
		...
		if (gu8App == APP_AP)
		{
			M2M_PRINT("STA IP Address \"%u.%u.%u.%u\"\n",pu8IPAddress[0],pu8IPAddress[1],pu8IPAddress[2],pu8IPAddress[3]);
			led_ctl(LED_AP, ON);
#ifdef LED_BTN_DEMO
			LedBtnDemo_AppStart(tcp_cb);
			u8Socket_Opened = 1;
#endif
		}
		else
	...
	else if (u8MsgType == M2M_WIFI_RESP_CLIENT_INFO)
	{
		...
		{
			led_ctl(LED_DEMO3, ((on & LED_DEMO3) ? (OFF) : (ON)));
		}
		else if (*pstrInfo == M2M_CLIENT_RESP_BTN1_PRESS)
		{
			led_ctl(LED_DEMO1, ((on & LED_DEMO1) ? (OFF) : (ON)));
		}
		else if (*pstrInfo == M2M_CLIENT_RESP_BTN2_PRESS)
		{
			led_ctl(LED_DEMO2, ((on & LED_DEMO2) ? (OFF) : (ON)));
		}
	}
...
void App_ProcessActRequest(tenuActReq enuActReq)
{
	...
	if (enuActReq == ACT_REQ_CONNECT_PROV)
	{
		gu8Sleep = PS_WAKE;
		M2M_REQ("ACT_REQ_CONNECT_PROV\n");
		//m2m_wifi_default_connect();
		app_wifi_start_stop_prov();
	}
	else if (enuActReq == ACT_REQ_WPS_CONNECT)
	{
		gbWifiConnected = M2M_WIFI_DISCONNECTED;
		//led_ctl(LED_ERR,ON);
		M2M_REQ("ACT_REQ_WPS_CONNECT\n");
		led_ctl(LED_WIFI,BLINK);
		m2m_wifi_wps(4,0);
		gbWpsActive = WPS_ENABLED;
		gu8Sleep = PS_WAKE;
	}
#ifdef GROWL
	else if (enuActReq == ACT_REQ_START_PROWL_APP)
	{
		gu8Sleep = PS_WAKE;
		gbProwlActive = 1;
		M2M_REQ("ACT_REQ_START_PROWL_APP\n");
		NMI_GrowlSendNotification(PROWL_CLIENT,(uint8*)"NMIApp",(uint8*)"NMI_Event",(uint8*)"Win PC",PROWL_CONNECTION_TYPE);
		led_ctl(LED_GRWOL, BLINK);
	}
	else if (enuActReq == ACT_REQ_START_NMA_APP)
	{
		gu8Sleep = PS_WAKE;
		gbNmaActive = 1;
		M2M_REQ("ACT_REQ_START_NMA_APP\n");
		NMI_GrowlSendNotification(NMA_CLIENT,(uint8*)"NMIApp",(uint8*)"NMI_Event",(uint8*)"Win PC",NMA_CONNECTION_TYPE);
		led_ctl(LED_GRWOL, BLINK);
	}
	else if (enuActReq == ACT_REQ_START_BOTH_APP)
	{
		gu8Sleep = PS_WAKE;
		gbProwlActive = 1;
		gbNmaActive = 1;
		M2M_REQ("ACT_REQ_START_BOTH_APP\n");
		NMI_GrowlSendNotification(PROWL_CLIENT,(uint8*)"NMIApp",(uint8*)"NMI_Event",(uint8*)"Win PC",PROWL_CONNECTION_TYPE);
		NMI_GrowlSendNotification(NMA_CLIENT,(uint8*)"NMIApp",(uint8*)"NMI_Event",(uint8*)"Win PC",NMA_CONNECTION_TYPE);
		led_ctl(LED_GRWOL, BLINK);
	}
#endif
	else if (enuActReq == ACT_REQ_DISCONNECT)
	{
		M2M_REQ("ACT_REQ_DISCONNECT\n");
		gu8Sleep = PS_WAKE;
		m2m_wifi_disconnect();
	}
	else if (enuActReq == ACT_REQ_CONNECT_LIST)
	{
		M2M_REQ("ACT_REQ_CONNECT_LIST\n");
		gu8ConnectList = 1;
		gu8Sleep = PS_WAKE;
		m2m_wifi_request_scan(M2M_WIFI_CH_ALL);
	}
	else if (enuActReq == ACT_REQ_UDP_TEST)
	{
#ifdef UDP_TEST
		gu8Sleep = PS_WAKE;
		M2M_REQ("ACT_REQ_UDP_TEST\n");
		SocketTest();
#else
	...
	else if (enuActReq ==  ACT_REQ_P2P_CONNECT)
	{
		M2M_REQ("ACT_REQ_P2P_CONNECT\n");
		m2m_wifi_p2p(M2M_WIFI_CH_1);
		gu8App = APP_P2P;
		gu8Sleep = PS_WAKE;
	}
	else if (enuActReq == ACT_REQ_P2P_DISCONNECT)
	{
		M2M_REQ("ACT_REQ_P2P_DISCONNECT\n");
		m2m_wifi_p2p_disconnect();
		gu8App = APP_WIFI;
		gu8Sleep = PS_WAKE;
	}

	else if (enuActReq == ACT_REQ_TCP_SEND)
	{
#ifdef LED_BTN_DEMO
		gu8Sleep = PS_WAKE;
		M2M_REQ("ACT_REQ_TCP_SEND\n");
		LedBtnDemo_AppSendNotification("Movement Detected");
#endif
	}
	else if (enuActReq == ACT_REQ_ENABLE_AP)
	{
		m2m_wifi_enable_ap(&gstrM2MAPConfig);
		gu8Sleep = PS_WAKE;
		gu8App = APP_AP;
	}
	else if (enuActReq == ACT_REQ_DISABLE_AP)
	{
		M2M_REQ("ACT_REQ_DISABLE_AP\n");
		m2m_wifi_disable_ap();
		gu8Sleep = PS_WAKE;
		gu8App = APP_WIFI;
	}
...
void app_main_context(void* pv)
{
	if(gu8App == APP_WIFI)
	{
		genuActReq = ACT_REQ_CONNECT_PROV;
		//gbWifiConnectInprogress = 1;
		M2M_INFO("WIFI APP\n");
	}
	else if(gu8App == APP_AP)
	{
